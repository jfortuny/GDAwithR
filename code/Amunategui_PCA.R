...
# Run the top 5 variables through PLS and then measure the ROC/AUC
...
# pls model
set.seed(1234)
pls_model <- train(Survived~., data=traindf_truncated, 
                   method="pls",
                   metric='ROC', 
                   trControl=objControl)
pls_predictions <- predict(object=pls_model,
                           testdf[,setdiff(names(traindf_truncated), 'Survived')], 
                           type='prob')
library(pROC)
print(auc(predictor=pls_predictions[[2]],
          response=ifelse(testdf$Survived=='yes',1,0)))

# we could also loop through models for final comparison of accuracy
...
for (method_name in method_names) {
  ...
  predictions <- predict(object=model,
                         testdf[,setdiff(names(traindf_truncated), 'Survived')], 
                         type='prob')
  print(auc(predictor=predictions[[2]],
            response=ifelse(testdf$Survived=='yes',1,0)))
}
